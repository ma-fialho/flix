    ...
    for (stratum <- root.strata) {
      for (constraint <- stratum.constraints) {
        reachableFunctions ++= (constraint.head match {
          ...
          case SimplifiedAst.Predicate.Head.Table(sym, terms, loc) => visitHeadTerms(terms)
        })

        reachableFunctions ++= constraint.body.map {
          case SimplifiedAst.Predicate.Body.Table(sym, polarity, terms, loc) => visitBodyTerms(terms)
